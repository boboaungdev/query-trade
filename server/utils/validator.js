...
export const validateOptionalToken = () => {
  return async (req, res, next) => {
...
export const validateCookie = () => {
  return async (req, res, next) => {
    ...
    if (!refreshToken) {
      return next(resError(400, "Session expired. Please sign in again."));
    }
    ...
    if (!decodedUser) {
      clearCookie(req, res, "refreshToken");
      return next(
        resError(404, "Authenticated user not found. Please sign in again."),
      );
    }
    ...
    if (decodedUser.refreshToken !== refreshToken) {
      ...
      return next(
        resError(
          403,
          "Session expired or signed in on another device. Please sign in again.",
        ),
      );
    }
